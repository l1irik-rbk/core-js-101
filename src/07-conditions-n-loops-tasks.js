...
function isCreditCardNumber(ccn) {
  const newCcn = ccn.toString().split('');
  const newArr = [];

  for (let i = newCcn.length - 1; i >= 0; i -= 1) {
    if (i % 2 !== 0) {
      let newDigit = +newCcn[i] * 2;
      if (newDigit > 9) newDigit -= 9;
      newArr.push(newDigit);
    } else {
      newArr.push(+newCcn[i]);
    }
  }
  // console.log(newCcn)
  // console.log(newArr.reverse())
  const sum = newArr.reverse().reduce((a, b) => a + b, 0);
...
function getDigitalRoot(num) {
  let newNum = num;
  while (newNum > 10) {
    newNum = newNum.toString().split('').reduce((a, b) => +a + +b, 0);
  }
  return newNum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (OPEN_KEYS.includes(currentSymbol)) {
      newArr.push(currentSymbol);
    } else {
      const topElement = newArr[newArr.length - 1];

      if (PARE_KEYS[currentSymbol] === topElement) {
        newArr.pop();
      } else {
        return false;
      }
